...
\subsection{Data (group: \inl{/data/})}
\begin{multicols}{2}
...
\inlvar{dataTD} & \inltab{Number} & $N \times J \times C \times Z$ & & yes & Measurement data stored in time domain representation \\ \hline 
...
\inlvar{snrFD} & \inltab{Float64} & $J \times C \times K$ &  & yes & Signal-to-noise estimate for recorded frequency components \\ \hline
...
\noindent \begin{tabularx}{\columnwidth}{lllllX} 
\textbf{Parameter} & \textbf{Type} & \textbf{Dim} & \textbf{Unit/Format} & \textbf{Optional} & \textbf{Description} \\ \hline 
\inlvar{data} & \inltab{Number} & $L\times N \times S$ & & yes & Reconstructed data \\ \hline
...
\begin{itemize}
	...
	\item Added \inlvar{data} subgroup to \inlvar{measurements} and restructured storage of raw measurement data.
