...
The term \textit{frame} refers to the data collected in one acquisition cycle. Usually all data within a frame will be combined to reconstruct a single MPI image/tomogram. In the simplest scenario, this data is acquired during one \inlvar{drivefield/cycle}. The acquisition time increases by a factor of \inlvar{numAverages} if block-averaging is applied. One averaged drive-field cycle will be referred to as one \textit{drive-field period}. The drive-field period is thus the product of \inlvar{drivefield/cycle} and \inlvar{numAverages}.

In case that the static selection field is changed over time one will measure several drive-field periods each with a different gradient field setting. In a classical multi-patch sequence the gradient field will be shifted in space from period to period. A full frame consists of \inlvar{numPeriodsPerFrame} ($J$) periods. The gradient settings within each period are described by the fields \inlvar{/acquisition/gradient} and \inlvar{/acquisition/offsetField}. \inlvar{/acquisition/gradient} has $J \times Y \times 3 \times 3$ entries whereas \inlvar{/acquisition/offsetField} is of dimension $J \times Y \times 3$. While $J$ describes the current drive-field period, $Y$ can be used to describe sequences where the gradient and offset field change during a drive-field period. Each period is split into $Y$ equidistantly spaced time intervals whereas the values stored in \inlvar{/acquisition/gradient} and \inlvar{/acquisition/offsetField} correspond to the beginning of each time interval. In case $Y=1$ the fields at the beginning of the period $J$ are provided, which can be used to describe a static multi-patch sequence. Higher numbers of $Y$ allow the description of an arbitrarily fine grained gradient and offset field sequence.
