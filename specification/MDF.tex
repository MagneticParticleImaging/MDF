...
\setbox\ltmcbox\vbox{
	...
	\noindent \begin{longtable}{p{0.12\columnwidth}p{0.8\columnwidth}} 
		...
		$E$ &  acquired background frames\\ \hline
		$O$ &  acquired frames without background frames ($O=N-E$)\\ \hline
		$B$ &  coefficients stored after basis transformation ($B\leq O$)\\ \hline
...
The motivation for reducing the number of frequency components is the reduction of the file size. Another, even more effective method is applying a (linear) transformation to the data along the frame dimension and storing only the most significant of the resulting coefficients. This option is only available if \inlvar{isFastFrameAxis}, \inlvar{isFourierTransformed}, and \inlvar{isFramePermutation} are all set to true. The frames need to be permuted in such a way that first $O$ frames are the foreground frames and the last $E$ frames are the background frames. The basis transformation is only applied to the $O$ foreground frames. In case of compressed data, the flag \inlvar{isBasisTransformed} is set to \inltab{true} and two additional fields \inlvar{basisTransformation} and \inlvar{basisIndices} need to be stored. \inlvar{basisTransformation} contains the name of the applied transformation, where we use the following standard identifiers: DCT-I, DCT-II, DCT-III, DCT-IV, FFT all of which have standard meaning. Additional transformations are non-standard and need to be integrated into this specification prior usage. In case that the $O$ foreground frames lay on a 2D or 3D grid (see \inlvar{/calibration/size}) we consider the multidimensional versions of the transformations. The basis transformation applied along the $O$ foreground frames yields $O$ coefficients. One then applies a threshold and stores only the $B\leq O$ most significant coefficients. The indices of these coefficients are stored in \inlvar{basisIndices}.
...
\inlvar{basisIndices} & \inltab{Integer} & $ J \times C \times W \times B$ & \inlvar{isBasisTransformed} & Indices of the coefficients (between $1$ and $O$)\\ \hline
\inlvar{basisTransformation} & \inltab{String} & 1 & \inlvar{isBasisTransformed} & Name of the transformation that has been applied\\ \hline
...
\inlvar{isBasisTransformed} & \inltab{Int8} & 1 & no & Flag, if the data is transformed along the (foreground) frame dimension \\ \hline
