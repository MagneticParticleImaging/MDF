...
\inlvar{isBackgroundData} & \inltab{Int8} & $N+M$ & & yes & TODO \\ \hline 
\end{tabularx}
...
\noindent \begin{tabularx}{\columnwidth}{llp{3cm}llX} 
\textbf{Parameter} & \textbf{Type} & \textbf{Dim} & \textbf{Unit/Format} & \textbf{Optional} & \textbf{Description} \\ \hline 
\inlvar{processedData} & \inltab{Number} & $N \times J \times C \times K\times 2$ & & yes & Processed data stored in frequency domain representation \\ \hline 
\inlvar{backgroundCorrected} & \inltab{Int8} & 1 & & yes & flag indicating if a background has been corrected \\ \hline  
\inlvar{spectralLeakageCorrected} & \inltab{Int8} & 1 & & yes & flag indicating if spectral leakage correction has been applied \\ \hline
\inlvar{transferFunctionCorrected} & \inltab{Int8} & 1 & & yes & flag indicating if the transfer function has been corrected \\ \hline 
\end{tabularx} 
...
\noindent \begin{tabularx}{\columnwidth}{llp{3cm}llX} 
\textbf{Parameter} & \textbf{Type} & \textbf{Dim} & \textbf{Unit/Format} & \textbf{Optional} & \textbf{Description} \\ \hline 
\inlvar{systemMatrixData} & \inltab{Number} & $J \times C \times K \times Q \times 2$  & & yes & Stores the background corrected system matrix in Fourier representation with the last dimension storing the complex data. \\  \hline
...
\inlvar{dataPositionIndex} & \inltab{Int64} & $N$ & m & yes & Order in which the positions have been acquired \\ \hline
