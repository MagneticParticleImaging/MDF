...
\item In v1.x the MDF allowed certain fields to have varying dimensions depending on the context. This has been removed such that starting from v2.0 all dimensions have to be specified. This change makes implementations of the MDF less complex. 
...
\inlvar{gradient} & \inltab{Float64} & $J \times 3$ & Tm$^{-1}\mu_0^{-1}$ & no & Gradient strength of the selection field in $x$, $y$, and $z$ directions \\ \hline
...
$$
where $F$ is the number of frequencies on the channel, $A_l$ is the drive-field strength, $\phi_l$ is the phase, $f_l$ is the frequency (described by the base frequency and the divider), and $\Lambda_l$ is the waveform. The waveform is specified by a dedicated parameter \inl{waveform}. If  \inl{waveform} is set to ``CUSTOM'', one can specify a custom waveform using the parameter \inl{customWaveform}.

%Depending on the base frequency and the divider a periodic excitation signal is generated defining the sampling trajectory of the field-free point or field-free line. The trajectory may cover a 1D, 2D, or 3D area.
...
\inlvar{waveform} & \inltab{String} & $D \times F$ & 1 & no & Waveform type: ``SINE'', ``TRIANGLE'' or ``CUSTOM'' \\ \hline
...
\inlvar{numAverages} & \inltab{Int64} & 1 & & no & Number of internal averages (applied in hardware/software) \\ \hline
\inlvar{repetitionTime} & \inltab{Float64} & 1 & s & no & Time to complete averaged DF trajectory (averages * period)\\ \hline
\inlvar{fieldOfView} & \inltab{Float64} & $J\times 3$ & m & no & Approximate size of the area/volume captured by the trajectory \\ \hline
\inlvar{fieldOfViewCenter} & \inltab{Float64} & $J \times 3$ & m & no & Center of the drive field trajectory (relative to origin/center) \\ \hline
\inlvar{offsetField} & \inltab{Float64} & $J \times 3$ & T$\mu_0^{-1}$ & yes & Applied offset field that yielded the fieldOfViewCenter\\ \hline
\end{tabularx}
...
One measurement consists of the voltages recorded in all receive channels for all patches. The number of measurements in time domain is thus given by $J C Z$. In frequency domain one measurement has $K C Z$ data points. On disc the temporal/frequency index is the fastest, while the index over the patches is the slowest. If several measurements are acquired (indicated by \textit{numFrames}), the measurements are concatenated. The number of measurements is denoted by $L$. \newline

\end{multicols}

\noindent \begin{tabularx}{\columnwidth}{llp{3cm}llX} 
\textbf{Parameter} & \textbf{Type} & \textbf{Dim} & \textbf{Unit/Format} & \textbf{Optional} & \textbf{Description} \\ \hline 
\inlvar{dataFD} & \inltab{Any} & $L \times J \times C \times K \times 2$  & & yes & Measurement data stored in Fourier domain representation. The last dimension is used for storing the complex data. \\  \hline
\inlvar{dataTD} & \inltab{Any} & $L \times J \times C \times Z$ & & yes & Measurement data stored in time domain representation \\ \hline 
\end{tabularx}
...
\noindent \begin{tabularx}{\columnwidth}{llp{3cm}llX} 
\textbf{Parameter} & \textbf{Type} & \textbf{Dim} & \textbf{Unit/Format} & \textbf{Optional} & \textbf{Description} \\ \hline 
\inlvar{dataTimeOrder} & \inltab{Int64} & $N$ & & yes & Time ordering number for calibration measurements in system matrix \\ \hline
\inlvar{dataFD} & \inltab{Any} &  $J \times C \times K \times N \times 2$ & & yes & System matrix stored in its Fourier space representation with the last dimension storing complex data \\ \hline
\inlvar{dataTD} & \inltab{Any} &  $J \times C \times Z \times N$ & & yes & System matrix stored in its time domain representation \\ \hline 
\inlvar{backgroundDataTimeOrder} & \inltab{Int64} & $M$ & & yes & Time ordering number for background measurements \\ \hline
\inlvar{backgroundDataFD} & \inltab{Any} & $M \times J \times C \times K \times 2$ & & yes & $M$ background measurements stored in Fourier space representation with the last dimension storing complex data \\ \hline
\inlvar{backgroundDataTD} & \inltab{Any} &  $M \times J \times C \times Z$ & & yes & background measurements stored in time domain representation \\ \hline 
...
\inlvar{offsetField} & \inltab{Float64} & $N \times 3$ & T$\mu_0^{-1}$ & yes & Applied offset field strength to emulate a spatial position, stored as ($x$, $y$, $z$) triples \\ \hline
...
\noindent \begin{tabularx}{\columnwidth}{lllllX} 
\textbf{Parameter} & \textbf{Type} & \textbf{Dim} & \textbf{Unit/Format} & \textbf{Optional} & \textbf{Description} \\ \hline 
\inlvar{data} & \inltab{Any} & $L\times N \times S$ & & yes & Reconstructed data \\ \hline
